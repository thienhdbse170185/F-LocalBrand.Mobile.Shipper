...
class RecievedTab extends StatefulWidget {
  const RecievedTab({Key? key}) : super(key: key);

  @override
  State<RecievedTab> createState() => _RecievedTabState();
}

class _RecievedTabState extends State<RecievedTab> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<OrderCubit, OrderState>(
      listener: (context, state) {
        if (state is GetShipperReceivedOrdersInprogress) {
          setState(() {
            _isLoading = true;
          });
        } else if (state is GetShipperReceivedOrdersSuccess) {
          setState(() {
            orders = state.orders;
            _isLoading = false;
            _isEmpty = orders!.isEmpty; // Check if orders list is empty
          });
        } else if (state is GetShipperReceivedOrdersEmpty) {
          setState(() {
            _isLoading = false;
            _isEmpty = true;
          });
